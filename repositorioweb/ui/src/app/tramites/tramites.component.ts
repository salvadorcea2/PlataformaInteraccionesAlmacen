...
export class TramitesComponent implements OnInit {
  ...
  public _items: Tramite[] = [];
  public _item_eliminar: Tramite = new Tramite({});
    ...
    this._apiService.loadBuscador(this._component_entidad + '.buscador')
      .subscribe(
        (data: any[]) => {
          ...
          if ( data.length > 0 ) {
            for (const d of data) {
              this._items.push( new Tramite(d) );
            }
          }
        },
        (error) => this._alertasService.onNuevaAlerta.emit(new Alerta({type: 'danger', dismissible: true, istimeout: true, timeout:10000, msg: 'Error al buscar los trámites'}))
      );
  }
  ...
  onEliminar (template: TemplateRef<any>, item: Tramite) {
  ...
  _onEliminar () {
    ...
    this._apiService.borrar(this._component_entidad + '/' + this._item_eliminar.id)
      .subscribe(
        (data: any) => this._alertasService.onNuevaAlerta.emit(new Alerta({type: 'success', dismissible: true, istimeout: true, timeout:10000, msg: 'Se eliminó el trámite "' + this._item_eliminar.nombre + '" con éxito'})),
        (error) => this._alertasService.onNuevaAlerta.emit(new Alerta({type: 'danger', dismissible: true, istimeout: true, timeout:10000, msg: 'Error al eliminar el trámite "' + this._item_eliminar.nombre + '"'})),
        () => {
          this._item_eliminar = new Tramite({});
          this.updateItems();
        }
      );
  }
