...
export class UsuariosComponent implements OnInit {
  ...
  public _items: Usuario[] = [];
  public _item_eliminar: Usuario = new Usuario({});
    ...
    this._apiService.loadBuscador(this._component_entidad + '.buscador')
      .subscribe(
        (data: any[]) => {
          ...
          if ( data.length > 0 ) {
            for (const d of data) {
              this._items.push( new Usuario(d) );
            }
          }
        },
        (error) => this._alertasService.onNuevaAlerta.emit(new Alerta({type: 'danger', dismissible: true, istimeout: true, timeout:10000, msg: 'Error al buscar los usuarios'}))
      );
  }
  ...
  onEliminar (template: TemplateRef<any>, item: Usuario) {
  ...
  _onEliminar () {
    ...
    this._apiService.borrar(this._component_entidad + '/' + this._item_eliminar.id)
      .subscribe(
        (data: any) => this._alertasService.onNuevaAlerta.emit(new Alerta({type: 'success', dismissible: true, istimeout: true, timeout:10000, msg: 'Se eliminó el usuario "' + this._item_eliminar.getNombre() + '" con éxito'})),
        (error) => this._alertasService.onNuevaAlerta.emit(new Alerta({type: 'danger', dismissible: true, istimeout: true, timeout:10000, msg: 'Error al eliminar el usuario "' + this._item_eliminar.getNombre() + '"'})),
        () => {
          this._item_eliminar = new Usuario({});
          this.updateItems();
        }
      );
  }
  ...
  ngOnInit() {
    ...
    if ( typeof this._buscador.filtro.tipo === 'undefined' || this._buscador.filtro.tipo === null ) {
      this._buscador.filtro.tipo = '';
    }
    if ( typeof this._buscador.filtro.rol === 'undefined' || this._buscador.filtro.rol === null ) {
      this._buscador.filtro.rol = '';
    }
